...
def create_root(
    ...
    source_label.dnd_bind(
        "<<Drop>>", lambda event: select_source_path(event.data)
    )
    ...
    target_label.dnd_bind(
        "<<Drop>>", lambda event: select_target_path(event.data)
    )
    ...
    keep_fps_checkbox = ctk.CTkSwitch(
        ...
        command=lambda: setattr(
            roop.globals, "keep_fps", not roop.globals.keep_fps
        ),
    )
    ...
    keep_frames_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: setattr(
            roop.globals, "keep_frames", keep_frames_value.get()
        ),
    )
    ...
    skip_audio_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: setattr(
            roop.globals, "skip_audio", skip_audio_value.get()
        ),
    )
    ...
    many_faces_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: setattr(
            roop.globals, "many_faces", many_faces_value.get()
        ),
    )
    ...
    donate_label.bind(
        "<Button>",
        lambda event: webbrowser.open("https://github.com/sponsors/s0md3v"),
    )
...
def render_image_preview(
    image_path: str, size: Tuple[int, int]
) -> ctk.CTkImage:
...
def init_preview() -> None:
    ...
    if is_video(roop.globals.target_path):
        ...
        if video_frame_total > 0:
            ...
            PREVIEW.bind(
                "<Right>",
                lambda event: update_frame(int(video_frame_total / 20)),
            )
            PREVIEW.bind(
                "<Left>",
                lambda event: update_frame(int(video_frame_total / -20)),
